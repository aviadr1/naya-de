...
def procss_row(events):
    ...
    sql = insert_statement.format(
        events["vendorid"],	            events["pickup_datetime"],	    events["dropoff_datetime"],
        events["store_and_fwd_flag"],	events["ratecodeid"],       events["passenger_count"],
        events["trip_distance"],	    events["PaymentType"]
    )
    # print(sql)
    mysql_conn.commit()
