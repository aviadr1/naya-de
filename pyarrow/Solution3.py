...
print(str(round(fs.disk_usage('hdfs://Cnt7-naya-cdh63:8020/user') / pow(1024, 2), 0)) + ' MB')
...
# 6. Rename a file in HDFS and check if the file was renamed.
fs.rename('hdfs://Cnt7-naya-cdh63:8020/tmp/sqoop/staging/ordsTable.sql',
                                'hdfs://Cnt7-naya-cdh63:8020/tmp/sqoop/staging/ordsTable_new.sql')
fs.ls('hdfs://Cnt7-naya-cdh63:8020/tmp/sqoop/staging', detail=False)

# 7. Delete the whole directory /tmp/sqoop/staging and it's files and check directory again.
fs.rm(f'hdfs://Cnt7-naya-cdh63:8020/tmp/sqoop/staging', recursive=True)
...
with open('/home/naya/tutorial/sqoop/scripts/file.txt','rb') as f:
    fs.upload('hdfs://Cnt7-naya-cdh63:8020/tmp/sqoop/development/file.txt', f, buffer_size=None)
